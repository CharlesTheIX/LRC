...
pub const PauseScreen = struct {
    ...
    // Base methods
    pub fn deinit(self: *PauseScreen) void {
        rl.unloadTexture(self.background_texture);
    }

    pub fn draw(self: *PauseScreen) void {
        self.drawBackground();
        self.drawMenu();
    }

    pub fn init(props: Props) PauseScreen {
        return PauseScreen{
            .font = props.font,
    ...
    pub fn load(self: *PauseScreen, game: *Game) void {
    ...
    pub fn update(self: *PauseScreen, game: *Game) void {
        _ = self;
        ...
        if (kb.activeKeysInclude(&[_]Key{.G}, .Or)) game.setState(.Playing);
        if (kb.activeKeysInclude(&[_]Key{.J}, .Or)) game.setState(.Settings);
    }

    // Helper methods
    fn drawBackground(self: *PauseScreen) void {
    ...
    fn drawMenu(self: *PauseScreen) void {
        ...
        const play_game_txt = "Press G to Play Game";
